...
internal static class ModConfig {
    ...
    internal static void Init(ConfigFile config) {
        ...
        CartCapacityEnabled = config.Bind("Cart Capacity", "Enabled", true,
            new ConfigDescription("Sets how many items the vanilla carts can carry - modded carts are not supported. A cart's value is its base capacity, and the Mercury blessing adds the bonus below on top, so 0 means a cart that carries nothing. High values can cause stutter and stack the cargo into a tall tower above the cart. Lowering a value does not take cargo off a cart: it keeps what it carries and only stops picking up more, until you unload it by hand. In multiplayer the host's values apply to everyone, and every player needs the mod installed to SEE cargo beyond the normal four. Raising a cart above its normal capacity is the only thing this mod writes to your save: turn this feature off and load each affected world once before uninstalling.", null,
                SectionTag("Cart Capacity", 4), EntryTag("Enabled", 0)));
        CartCapacityBlessingBonus = config.Bind("Cart Capacity", "Blessing Bonus", 1,
            new ConfigDescription("How much the Mercury cart-capacity blessing adds on top of a cart's base capacity.", new AcceptableValueRange<int>(0, 20),
